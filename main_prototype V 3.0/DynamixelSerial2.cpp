...
void DynamixelClass::switchCom(unsigned char Mode)
{
	if(Mode == Rx_MODE)
	{
		//digitalWrite(Direction_Pin_TX, HIGH);
		//digitalWrite(Direction_Pin_RX, LOW);
		PORTJ |= 0x01;
		PORTJ &= 0xFD;
	}
	else
	{
		//digitalWrite(Direction_Pin_TX, LOW);
		//digitalWrite(Direction_Pin_RX, HIGH);
		PORTJ &= 0xFE;
		PORTJ |= 0x02;
	}
}
...
void DynamixelClass::begin(long baud, unsigned char txEN, unsigned char rxEN)
{	
	Direction_Pin_TX = txEN;
	Direction_Pin_RX = rxEN;
	setDPin(Direction_Pin_TX, OUTPUT);
	setDPin(Direction_Pin_RX, OUTPUT);
	beginCom(baud);
}

void DynamixelClass::begin(long baud, unsigned char directionPin)
{	
	Direction_Pin_TX = directionPin;
	Direction_Pin_RX = -1;
	setDPin(Direction_Pin_TX,OUTPUT);
	beginCom(baud);
}	
